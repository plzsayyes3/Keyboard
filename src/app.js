import { createLayoutIndex, resolveExpectedKeys } from './engine/layout.js';
...
import { renderApp, renderTask } from './ui.js?v=20260919-youon1';

const root = document.querySelector('#app'); const layoutPaths = ['data/layouts/naginata-v18.json?v=20260919-youon1', 'data/layouts/user-current.json?v=20260919-youon1']; const lessonPaths = ['data/lessons/basic.json', 'data/lessons/words.json', 'data/lessons/sentences.json'];
...
async function boot() {
  ...
  const state = { layouts, layout: layouts[0], lessons, lesson: lessons[0], mode: 'site', session: new PracticeSession(lessons[0].items), feedback: '練習を始めると結果が表示されます。', feedbackKind: '', heldKeys: [], expectedKeys: [] }; state.expectedKeys = resolveExpectedKeys(createLayoutIndex(state.layout), state.session.current.expected); renderApp(root, state); wireControls(state); attachInput(state);
}
...
function resetSession(state, message) { state.session = new PracticeSession(state.lesson.items); state.feedback = message; state.feedbackKind = ''; state.heldKeys = []; state.expectedKeys = resolveExpectedKeys(createLayoutIndex(state.layout), state.session.current.expected); renderTask(root, state); }
function attachInput(state) {
  ...
  const engine = new NaginataEngine(createLayoutIndex(state.layout)); state.controller = createInputController({ mode: state.mode, engine, onHeldKeys: (keys) => { state.heldKeys = keys; renderTask(root, state); }, onOutput: (value) => { const result = state.session.submit(value); state.feedback = result.correct ? `○ ${value}　正解` : `× ${value || '未定義'}　正しくは ${result.expected}`; state.feedbackKind = result.correct ? 'success' : 'error'; state.expectedKeys = result.completed ? [] : resolveExpectedKeys(createLayoutIndex(state.layout), state.session.current.expected); renderTask(root, state); } });
